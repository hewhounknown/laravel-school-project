    ...
    <div class="row my-5 fs-4">
        <div class="col">
            @foreach ($categories as $cat)
                <a id="{{ $cat->id }}"
                    class="category badge text-black-50 border btn inline">{{ $cat->category_name }}</a>
            @endforeach
        </div>
    </div>
                                    ...
                                    <div class="card h-100">
                                        @if ($c->course_image == null)
                                            <img src="{{ asset('img/default.png') }}" class="card-img-top img-fluid"
                                                style="" alt="...">
                                        @else
                                            <img src="{{ asset('storage/course/' . $c->course_image) }}"
                                                class="card-img-top img-fluid" style="" alt="...">
                                        @endif
                                        <div class="card-body" style="">
                                            <h5 class="card-title">{{ $c->course_name }}
                                                <span
                                                    class="badge text-bg-secondary">{{ $c->category->category_name }}</span>
                                            </h5>
                        ...
                        if (response.length != 0) {
                            ...
                            response.forEach(course => {
                                ...
                                if (course.course_image == null) {
                                    ...
                                    $list +=
                                        `<img src="{{ asset('storage/course/') }}./${course.course_image}" class="card-img-top img-fluid" style="" alt="..." >`;
                                }
