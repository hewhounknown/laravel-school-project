...
@section('content')
    ...
    <div class="container bg-body-tertiary p-2 position-relative">
        ...
        @if (Auth::user()->role == 'teacher')
            <span class="position-absolute top-0 start-100 translate-middle p-2 border border-light rounded-circle"
                ...
                <ul class="dropdown-menu">
                    ...
                    <li><a class="dropdown-item" href="http://" data-bs-toggle="modal"
                            data-bs-target="#deleteForm">Delete</a></li>
                </ul>
            </span>
            ...
            <div class="modal fade" id="editForm" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <form enctype="multipart/form-data" action="{{ route('teacher.content.edit', $content->id) }}"
                            method="post">
                            ...
                            <div class="modal-body">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="contentTitle" id="contentTitle" class="form-control"
                                        value="{{ old('contentTitle', $content->title) }}">
                                </div>

                                <div class="mb-3">
                                    <select id="selectBox" name="contentType" class="form-select choiceContentType"
                                        aria-label="Default select example">
                                        <option value="text" {{ $content->content_type == 'text' ? 'selected' : '' }}>Text
                                        </option>
                                        <option value="video" {{ $content->content_type == 'video' ? 'selected' : '' }}>
                                            video</option>
                                        <option value="image" {{ $content->content_type == 'image' ? 'selected' : '' }}>
                                            image</option>
                                        <option value="file" {{ $content->content_type == 'file' ? 'selected' : '' }}>file
                                        </option>
                                    </select>
                                </div>
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                            <a
                                href="{{ route('content.delete', ['topicId' => $content->topic->id, 'contentId' => $content->id]) }}"><button
        ...
        <div class="row m-auto">
            ...
            @elseif ($content->content_type == 'video')
                <div class="text-center">
                    <video width="800" height="420" controls
                        src="{{ asset('storage/course/topic/content/' . $content->content_path) }}"
            ...
            @else
                <div class="text-center">
                    <a href="{{ route('file.download', $content->content_path) }}">
                        <h3>{{ __('download file here!') }}</h3>
                    </a>
                </div>
            @endif
        </div>
    ...
    <div class="m-4">
        <ul class="list-group">
            <li class="list-group-item active" aria-current="true"><b>Comments <span
                        class="badge text-bg-info">{{ count($content->comments) }}</span> </b></li>
            @foreach ($content->comments as $comment)
                <li class="list-group-item">
                    <div class="fw-bold fst-italic">
                        <a href="{{ route('profile.view', $comment->user_id) }}"
                    ...
                    <div>{{ $comment->body }}
                        @if ($comment->user->id == Auth::user()->id)
                            <a href="{{ route('content.comment.delete', $comment->id) }}"
